...
export default function AdminReportesPage() {
  ...
  const productosBarData = useMemo(
    () => topProductos.map((p) => ({ name: p.nombre, cantidad: p.cantidad })),
    [topProductos],
  );
  const categoriasPieData = useMemo(
    () => topCategorias.map((c) => ({ name: c.nombre, value: c.cantidad })),
    [topCategorias],
  );
