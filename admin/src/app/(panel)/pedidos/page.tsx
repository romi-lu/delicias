...
export default function AdminPedidosPage() {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-col">
              <Link href={`/usuarios/${u.id}`} className="text-slate-900 hover:underline">
          ...
          return (
            <span
              className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                map[est] || "bg-slate-100 text-slate-700"
              }`}
            >
              {est || "—"}
            </span>
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.fecha_entrega ? (
            new Date(row.original.fecha_entrega).toLocaleDateString()
          ) : (
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const texto = `${p.id} ${p.usuario?.nombre || ""} ${p.usuario?.apellido || ""} ${p.usuario?.email || ""} ${
        p.notas || ""
      } ${p.direccion_entrega || ""}`.toLowerCase();
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        />
        <button className={buttonClasses({ variant: "outline-secondary", size: "sm" })} onClick={fetchPedidos}>
          Actualizar
        </button>
      </div>
