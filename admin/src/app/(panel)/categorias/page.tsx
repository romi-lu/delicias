...
export default function AdminCategoriasPage() {
  ...
  const toggleActivo = async (c: Categoria) => {
    try {
      const res = await axios.patch(`/api/categorias/admin/${c.id}/estado`, {
        activo: !c.activo,
      });
      const actualizado = res.data?.categoria || res.data;
      setCategorias((prev) =>
        prev.map((x) =>
          x.id === c.id ? { ...x, activo: actualizado?.activo ?? !c.activo } : x,
        ),
      );
      toast.success(
        actualizado?.activo ? "Categoría activada" : "Categoría desactivada",
      );
    } catch (e: unknown) {
      const errMsg = axios.isAxiosError(e)
        ? ((e.response?.data as { message?: string })?.message || e.message)
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <Link
              href={`/categorias/${row.original.id}`}
        ...
        <>
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="text-sm text-slate-600">
              ...
              {table.getPageCount()} — {filteredCategorias.length} categorías
            </div>
            <div className="flex items-center gap-2">
              <button
