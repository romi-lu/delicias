...
export default function EditarCategoriaPage() {
    ...
    try {
      if (file) {
        ...
        const imgRes = await axios.put(
          `/api/categorias/admin/${id}/imagen`,
          fd,
      ...
      if (!file && (form.imagen || "").trim()) {
        payload.imagen = form.imagen.trim();
      }
  ...
  const imagenActualSrc = useMemo(
    () => getImageSrc(form.imagen || categoria?.imagen || null, { width: 600 }),
    [form.imagen, categoria?.imagen],
  );
    ...
    <div>
      ...
      >
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Si subes un archivo local, se usará esa imagen y esta URL puede quedar vacía.
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className={buttonClasses({ variant: "outline-secondary", size: "md" })}
