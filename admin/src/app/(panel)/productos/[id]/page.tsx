      ...
      try {
        ...
        setCategorias(
          Array.isArray(catsData)
            ? catsData
            : Array.isArray((catsData as any)?.categorias)
              ? (catsData as any).categorias
              : [],
        );
        ...
        reset({
          ...
          imagen_url:
            p.imagen && String(p.imagen).startsWith("http") ? String(p.imagen) : "",
        });
        setError(null);
      } catch (e: unknown) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      if (file) {
        ...
        await axios.put(`/api/productos/${id}`, fd, {
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          {errors.categoria_id && (
            <p className="mt-1 text-xs text-red-600">{errors.categoria_id.message}</p>
          )}
        </div>
        <div>
        ...
        <div>
          ...
          <input
            type="file"
            accept="image/*"
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 shadow-sm"
        ...
        <div>
          ...
          />
          {errors.imagen_url && (
            <p className="mt-1 text-xs text-red-600">{errors.imagen_url.message}</p>
          )}
        </div>
        <div className="md:col-span-2 flex items-center gap-2">
          ...
          <button
            type="button"
            className={buttonClasses({ variant: "link", size: "md" })}
            onClick={() => router.back()}
          >
            Cancelar
          </button>
