    ...
    const fetchCategorias = async () => {
      try {
        ...
        const list = Array.isArray(data)
          ? data
          : Array.isArray((data as any)?.categorias)
            ? (data as any).categorias
            : [];
        setCategoriasList(
          list.map((c: any) => ({ id: Number(c.id), nombre: String(c.nombre || "") })),
        );
      } catch (e) {
        // no-op
      }
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      ...
      try {
        ...
        const data =
          res.data?.data || res.data?.productos || res.data?.items || res.data;
  ...
  const toggleDestacado = async (p: Producto) => {
    try {
      ...
      setProductos((prev) =>
        prev.map((item) =>
          item.id === p.id
            ? { ...item, destacado: actualizado?.destacado ?? !p.destacado }
            : item,
        ),
      );
      toast.success(
  ...
  const columns = useMemo<ColumnDef<Producto>[]>(
    () => [
          ...
          <div className="flex items-center gap-3">
            {row.original.imagen ? (
              <div className="h-14 w-14 overflow-hidden rounded-md border border-slate-200 bg-white flex-shrink-0">
                <Image
                  src={getImageSrc(row.original.imagen)}
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <span className="text-slate-500">{String(getValue() ?? "-")}</span>
        ),
      },
      {
        header: () => "Precio",
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <Badge
              variant={variant as "danger" | "warning" | "success"}
              size="md"
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/productos/nuevo" className={buttonClasses({ variant: "primary", size: "sm" })}>
          Nuevo producto
        </Link>
      </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {bajoStock.length > 0 && (
                <div>
                  ...
                  <span className="font-semibold">{bajoStock.length}</span>
                </div>
              )}
